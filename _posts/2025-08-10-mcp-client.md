...
2. distributed agent(A2A): 如果你怕一次要带太多的工具出门，消耗太多的token，那么你可以先由一个总管agent 决定调用的agent 库，每一个agent 都是某细分领域的专家，只携带对应领域的装备出门。Google的A2A协议，允许我们在agent之间进行沟通，这样，我们就能组队出门，有战士，法师，兽人，精灵，一起出门，各取所需，节约token。我们又可以通过一个工程方法，解决了token消耗的科学问题了。
