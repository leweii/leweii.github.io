...
通过code execution 我们能巧妙的够绕开，或者解决一些llm的局限。这是一个典型的用工程问题，解决科学问题的方案。
...
2. 分配云端资源，agent 可通过类似tool的形式对资源进行操作（aws lambda， docker container）
3. 在服务器内部操作cmd来执行一段代码（不推荐，可能被生成的代码反客为主）

这个给agent 写代码和执行代码的能力的解题思路，和把所有的tools作为上下文给到agent 的解题思路上，存在解决思路上的本质区别。前者是赋予agent能力，让他去以现有的资源来解决问题。后者更像是告诉他，你最好这样这样这样做，就能解决xxx问题。
...
Practice 的部分到一段路了，但是我觉得上文里，把agent 当作员工和学生之间的区别这件事，还是很值得拿出来再聊一聊的。我们很习惯当父母和老师，规定，操控和教授agent。这是好事，说明你知道一件事该如何做。但是随着需求的叠加，当我们遇到未知的流程，我们无法给agent一一指定所有的工作流程。每一个agent，都迟早有一天，要进入到自主解决问题的范围内。

让ai自主解决问题，不是放任ai自行发挥，自主解决问题，还是要完全在我们的控制之下自主。哪一天，ai不再需要人的控制（上下文工程等等），人就不再有尊严了。
